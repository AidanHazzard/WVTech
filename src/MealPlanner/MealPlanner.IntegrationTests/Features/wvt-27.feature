...
# WVT-27
  ...
  Scenario: Removing the only pantry item leaves the pantry empty
    ...
    And 'Gary' has a pantry item named 'Flour' with amount '3' and measurement 'Cup(s)'
  ...
  Scenario: Removing one item from a pantry with multiple items leaves the others intact
    Given 'Gary' has no pantry items
    And 'Gary' has a pantry item named 'Sugar' with amount '2' and measurement 'Cup(s)'
  ...
  Scenario: User increases the amount of a pantry item and the change is saved
    Given 'Gary' has no pantry items
    And 'Gary' has a pantry item named 'Rice' with amount '2' and measurement 'Cup(s)'
  ...
  Scenario: Setting a pantry item amount to zero shows a validation error
    Given 'Gary' has no pantry items
    And 'Gary' has a pantry item named 'Oats' with amount '3' and measurement 'Cup(s)'
  ...
  Scenario: Setting a pantry item amount to a non-number shows a validation error
    Given 'Gary' has no pantry items
    And 'Gary' has a pantry item named 'Honey' with amount '1' and measurement 'Count'
